...
public class RemotePrimaryRelocationIT extends MigrationBaseTestCase {
    ...
    public void testMixedModeRelocation_RemoteSeedingFail2() throws Exception {
        String docRepNode = internalCluster().startNode();
        ClusterUpdateSettingsRequest updateSettingsRequest = new ClusterUpdateSettingsRequest();
        updateSettingsRequest.persistentSettings(Settings.builder().put(REMOTE_STORE_COMPATIBILITY_MODE_SETTING.getKey(), "mixed"));
        assertAcked(client().admin().cluster().updateSettings(updateSettingsRequest).actionGet());
        ...
        remoteNodeTransportService.addRequestHandlingBehavior(
            PeerRecoveryTargetService.Actions.FINALIZE , (handler, request, channel, task) -> {
                if (failFinalize.get()) {
                    logger.info("action finalize received");
                    throw new IOException("");
                } else {
                    logger.info("going to pass .action finalize received" );
                    handler.messageReceived(request, channel, task);
                }
            }
        );

        //setBlockOnSegments(REPOSITORY_NAME);
//        client().admin()
//            .cluster()
//            .prepareUpdateSettings()
//            .setTransientSettings(Settings.builder().put(RecoverySettings.INDICES_INTERNAL_REMOTE_UPLOAD_TIMEOUT.getKey(), "20s"))
//            .get();
        ...
        // now unblock it
        logger.info("Unblocking the segment file now");
